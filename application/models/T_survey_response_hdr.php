...
**/
	...
	public function get_data_report_weekly()
	{
		...
		$query = $this->db->query('select `t_survey_response_hdr`.`int_pt`, `t_survey_response_hdr`.`dt_survey_complete`, `t_survey_response_hdr`.`consultant_id`, `t_company_info`.`nm_company`, `t_company_info`.`nm_designation`, `t_company_info`.`n_revenue`, `t_company_info`.`n_staff_size`, `t_company_info`.`n_hr_size`, `t_company_info`.`nm_industry` from `t_survey_response_hdr` join `t_company_info` on `t_survey_response_hdr`.`id_company` = `t_company_info`.`nm_company` where `t_survey_response_hdr`.`tx_status` = "Completed" and `t_survey_response_hdr`.`dt_survey_complete` between DATE_SUB(NOW(), INTERVAL 7 DAY) AND NOW() ORDER BY `t_company_info`.`nm_company`');
